...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # 在 _batch_level_generate_sequences 内部，生成 response 之后
        # if self._tp_rank == 0:
        #    # 获取当前 worker 的 logger
        #    logger = logging.getLogger(__name__)
            
        #    try:
        #        # 解码并构建我们要看的信息
        #        p_text = self.processing_class.decode(idx[0])
        #        r_text = self.processing_class.decode(response[0])
                
                # 构造一条极其醒目的巨长字符串
        #        debug_msg = (
        #            "\n" + "🚨" * 30 + "\n"
        #            f"【CRITICAL WORKER CHECK】\n"
        #            f"--- [PROMPT HEAD]: {p_text[:200].replace('\n', ' ')}\n"
        #            f"--- [RESPONSE HEAD]: {r_text[:200].replace('\n', ' ')}\n"
        #            + "🚨" * 30
        #        )
                
                # 重点：直接报 WARNING 级别，这在控制台绝对是高亮的
                # 甚至可以用 logger.error，只要能看到数据，不嫌级别高
        #        logger.warning(debug_msg)
                
        #        # 备招：如果 logger 还是被吞，直接写 stderr
        #        sys.stderr.write(debug_msg + "\n")
        #        sys.stderr.flush()

        #    except Exception as e:
        #        logger.error(f"❌ [DEBUG ERROR] Decoding failed in worker: {e}")
